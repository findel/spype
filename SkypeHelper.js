...
var SkypeHelper = 
{
	...
	skypeMessagesReceived : function(messages)
	{
		...
		{
            var spypeIsNotSender = (message.resource.from.toLowerCase().indexOf(config.skype_username.toLowerCase()) === -1) && (message.resource.from.toLowerCase().indexOf(config.live_username.toLowerCase()) === -1);
